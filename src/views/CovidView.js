        ...
        <div style={{ padding: "0 10px" }}>
            <Grid columns={2} stackable>
                <Grid.Column width={16}>
                    {loading ? <Loader active={loading} inline /> : <Graph monthData={monthData} />}
                </Grid.Column>
                <Grid.Column width={10} >
                    `{loading ? <Loader active={loading} inline /> : <Table tableData={tableData} />}
                </Grid.Column>
                <Grid.Column width={6} style={{ marginTop: "18px" }}>
                    {loading ? <Loader active={loading} inline /> : <PieChartContainer monthData={monthData} />}
                </Grid.Column>
            </Grid>
        </div>
    )
